module Subscriber
  ...
  class Message < Fb::Messenger::Subscriber::Base
    ...
    def call(event, opts = {})
      Shopify::Request.execute(event.sender_id, opts[:shop_session]) do
        ...
        if products && products.length > 0
          ...
          Fb::Messenger::Client.send_message_template(event.sender_id, template)
        else
          Fb::Messenger::Client.send_message_text(event.sender_id, NOT_FOUND)
        end
      end
    end
